...
var Range = Element.extend(vertical).extend({
  ...
  $.widget( "custom.tooltipX", $.ui.tooltip, {
    ...
    _open: function( event, target, content ) {
      ...
      var $c = this.$el.find('.elessar-barlabel').clone().css({display: 'inline', width: 'auto', visibility: 'hidden'}).appendTo('body');
      if($c.width() > this.$el.width()) {
	  if(false == this.$el.find('.elessar-barlabel').is("[aria-describedby]")) {
              this.$el.find('.elessar-barlabel').tooltipX({position: {my: "center bottom", at: "center top"}, items: "span", content: function(){return text;}, track: true, autoHide: false});
	      this.$el.find('.elessar-barlabel').tooltipX('open');
	  }
	  else {
	      this.$el.find('.elessar-barlabel').tooltipX('option', 'content', function(){return text;});
	      var id = this.$el.find('.elessar-barlabel').attr('aria-describedby');
	      $('#' + id).css('left', this.$el.offset().left);
	  }
      }
      else if(this.$el.find('.elessar-barlabel').is("[aria-describedby]")){ 
          this.$el.find('.elessar-barlabel').tooltipX("destroy");
      }
      $c.remove();
